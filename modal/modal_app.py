...
def _load_pipeline():
    ...
    controlnet = ControlNetModel.from_pretrained(CONTROLNET_MODEL, torch_dtype=dtype, use_safetensors=True, cache_dir=HF_CACHE_MOUNT)
    pipe = StableDiffusionXLControlNetPipeline.from_pretrained(SDXL_BASE, controlnet=controlnet, torch_dtype=dtype, use_safetensors=True, cache_dir=HF_CACHE_MOUNT)
    try: pipe.enable_vae_slicing()
    except: pass
    try: pipe.enable_sequential_cpu_offload() if not use_cuda else pipe.enable_model_cpu_offload()
    except: pass
    if use_cuda:
        pipe.to("cuda")
...
@app.function(image=image, timeout=900, network_file_systems={HF_CACHE_MOUNT: HF_CACHE}, gpu=GPU_ARG)
@web_endpoint("/stage", method="POST")
async def stage_http(request):
    from modal import Response
    try:
        if API_KEY and request.headers.get("x-api-key") != API_KEY:
            return Response(json.dumps({"error": "unauthorized"}).encode(), content_type="application/json", status_code=401)
        ...
        if "multipart/form-data" in content_type:
            ...
            if not uploaded:
                return Response(json.dumps({"error": "missing_file"}).encode(), content_type="application/json", status_code=400)
            ...
            if payload.get("image_url"):
                ...
                if not image_b64:
                    return Response(json.dumps({"error": "missing_image"}).encode(), content_type="application/json", status_code=400)
        ...
        return Response(json.dumps({"error": str(e)}).encode(), content_type="application/json", status_code=500)

@app.function(image=image, timeout=900, network_file_systems={HF_CACHE_MOUNT: HF_CACHE}, gpu=GPU_ARG)
def virtual_stage(image: bytes, room_style: str = "modern luxury", negative_prompt: str = "lowres, blurry", seed: Optional[int] = None, guidance_scale: Optional[float] = None, num_inference_steps: Optional[int] = None) -> bytes:
    from PIL import Image
    import torch
    ...
    generator = None
    if seed is not None:
        try: generator = torch.Generator(device=device).manual_seed(int(seed))
        except: generator = torch.Generator().manual_seed(int(seed))
    with torch.inference_mode():
        result = pipe(image=control, prompt=f"{room_style}, photorealistic", negative_prompt=negative_prompt, generator=generator, guidance_scale=g, num_inference_steps=steps)
    return _to_jpeg_bytes(result.images[0], JPEG_QUALITY)

@app.local_entrypoint()
def main():
    print("Use modal run modal_app.py::stage_http for HTTP endpoint")
