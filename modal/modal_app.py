...
# Keep-warm defaults (runtime-switchable via API)
DEFAULT_KEEPWARM_MODE = os.getenv("KEEPWARM_MODE", "off").lower()  # off | business | always
KEEPWARM_TZ = os.getenv("KEEPWARM_TZ", "America/New_York")
KEEPWARM_HOURS = os.getenv("KEEPWARM_HOURS", "09:00-18:00")  # local time window for business mode
KEEPWARM_WEEKDAYS = os.getenv("KEEPWARM_WEEKDAYS", "1-5")  # 1=Mon..7=Sun
...
image = (
    ...
    .pip_install(
        ...
        # "hf-transfer==0.1.7",  # optional: faster model pulls
    )
    .env(
        {
            # Use HF_HOME as the single source of truth for caches
            "HF_HOME": HF_CACHE_MOUNT,
            "HUGGINGFACE_HUB_CACHE": HF_CACHE_MOUNT,
            "DIFFUSERS_CACHE": HF_CACHE_MOUNT,
            "PYTHONUNBUFFERED": "1",
            # "HF_HUB_ENABLE_HF_TRANSFER": "1",  # if you add hf-transfer above
        }
    )
)
...
# ──────────────────────────────────────────────────────────────────────────────
# Keep-warm config + runtime switch
# ──────────────────────────────────────────────────────────────────────────────
CFG_DIR = os.path.join(HF_CACHE_MOUNT, "ssp_config")
KEEPWARM_CFG = os.path.join(CFG_DIR, "keepwarm.json")
...
def _read_keepwarm_cfg():
    _ensure_cfg_dir()
    if not os.path.exists(KEEPWARM_CFG):
        return {"mode": DEFAULT_KEEPWARM_MODE, "tz": KEEPWARM_TZ, "hours": KEEPWARM_HOURS, "weekdays": KEEPWARM_WEEKDAYS}
    try:
        with open(KEEPWARM_CFG, "r", encoding="utf-8") as f:
            cfg = json.load(f)
    except Exception:
        cfg = {}
    # fill defaults if keys missing
    cfg.setdefault("mode", DEFAULT_KEEPWARM_MODE)
    cfg.setdefault("tz", KEEPWARM_TZ)
    cfg.setdefault("hours", KEEPWARM_HOURS)
    cfg.setdefault("weekdays", KEEPWARM_WEEKDAYS)
    return cfg

def _write_keepwarm_cfg(cfg: dict):
    _ensure_cfg_dir()
    with open(KEEPWARM_CFG, "w", encoding="utf-8") as f:
        json.dump(cfg, f, ensure_ascii=False)

def _within_business_window(now_local, hours: str, weekdays: str) -> bool:
    # weekdays format like "1-5" or "1,2,3,4,5" (1=Mon..7=Sun)
    allowed = set()
    for part in str(weekdays).split(","):
        ...
        if "-" in part:
            ...
            try:
                ...
                pass
    # Python Mon=0..Sun=6 -> our 1..7
    weekday_num = now_local.weekday() + 1
    if allowed and weekday_num not in allowed:
        return False
    # hours format "HH:MM-HH:MM"
    try:
        start_s, end_s = [h.strip() for h in hours.split("-", 1)]
...
@app.function(
    image=image,
    gpu=GPU_ARG,
    timeout=1800,  # give first warm plenty of time to download models/cache
    network_file_systems=NFS_MOUNTS,
)
@modal.fastapi_endpoint(method="POST")
...
# ──────────────────────────────────────────────────────────────────────────────
# Keep-warm admin API + cron (runtime switch)
# ──────────────────────────────────────────────────────────────────────────────
@app.function(
    image=image,
    network_file_systems=NFS_MOUNTS,
...
@modal.fastapi_endpoint(method="GET")
async def keepwarm_status():  # no args
    from fastapi.responses import JSONResponse
    cfg = _read_keepwarm_cfg()
    return JSONResponse({"ok": True, "config": cfg}, status_code=200)
...
@modal.fastapi_endpoint(method="POST")
async def keepwarm_set(request: FastAPIRequest):  # body: {mode, tz?, hours?, weekdays?}
    from fastapi.responses import JSONResponse
    a = _auth(request)
    if not a["ok"]:
        return JSONResponse({"error": a["err"]}, status_code=a["status"])
    ...
    if mode not in {"off", "business", "always"}:
        return JSONResponse({"error": "invalid_mode", "allowed": ["off", "business", "always"]}, status_code=400)
    cfg = _read_keepwarm_cfg()
    cfg["mode"] = mode
    if "tz" in payload: cfg["tz"] = str(payload["tz"])
    if "hours" in payload: cfg["hours"] = str(payload["hours"])
    if "weekdays" in payload: cfg["weekdays"] = str(payload["weekdays"])
    _write_keepwarm_cfg(cfg)
    return JSONResponse({"ok": True, "config": cfg}, status_code=200)

# Cheap CPU cron that decides whether to trigger GPU warm
@app.function(
    image=image,  # CPU only here
    network_file_systems=NFS_MOUNTS,
    schedule=Cron("*/5 * * * *", timezone=KEEPWARM_TZ),  # every 5 minutes in configured TZ
)
def keepwarm_cron():
    # Read config
    cfg = _read_keepwarm_cfg()
    mode = cfg.get("mode", DEFAULT_KEEPWARM_MODE)
    tz = cfg.get("tz", KEEPWARM_TZ)
    hours = cfg.get("hours", KEEPWARM_HOURS)
    weekdays = cfg.get("weekdays", KEEPWARM_WEEKDAYS)

    if mode == "off":
        return

    # Decide if we should warm now
    try:
    ...
    should_warm = (mode == "always") or (mode == "business" and _within_business_window(now_local, hours, weekdays))
    if not should_warm:
        return

    # Trigger the GPU warm function asynchronously
    warm.remote()  # schedules a GPU container to load the pipeline
